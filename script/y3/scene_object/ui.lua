...
function ui:bind_format(format)
    game_api.set_ui_comp_bind_format(format)
end
...
function ui:set_adapt_option(direction, value)
    game_api.set_ui_comp_adapt_option(direction, value)
end

---@param  name string 事件名
---设置控件相对父级位置
function ui:trigger_ui_event(name)
    game_api.trigger_ui_event(name)
end

---@param  is_follow boolean 是否跟随
---设置控件是否跟随鼠标
function ui:set_follow(is_follow)
    game_api.set_ui_comp_follow_mouse(is_follow)
end